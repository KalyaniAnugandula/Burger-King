import React from "react";
import NavBarData from "./NavBarData";
import NavNone from "./NavNone";
function NavBar(){
     return(
         <>
         <nav>
          
               {/* <img src="	data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAKIA…wQnRYgROiwwqcEB1W8P+nbGklhzdU3gAAAABJRU5ErkJggg==" alt="burger" height={600} width={200}/> */}
        <img id="burger-icon" src="data:image/png;base64," alt="logo" class="header__logo-destop" width="79" height="79"></img>
        <div className="toggle">
         <span id="dine-in-text">DINE-IN/TAKEAWAY</span>
         <i className="fa-solid fa-toggle-on"></i> 
         <span id="deliv-text">Delivery</span> 
         <div id="seperator"></div>
        
         <a href="#" id="location"onClick={NavBarData} onDoubleClick={NavNone}>
         <img src="https://www.burgerking.in/static/media/location.b472e06f.svg" alt="" />
         <span id="pick-text">Tap on it to pick your location...</span>
            </a> 
          </div>   
          <ul className="nav-menu_list">
            <li><img src="	https://www.burgerking.in/static/media/dine-in-black.c32e1cdd.svg" alt="" /></li>
            <span>NEARBY STORES</span>
            <li><img src="	https://www.burgerking.in/static/media/offers-inactive.81dcb21f.svg" alt="" /></li>
            <span>KING DEALS</span>
            <li><img src="	https://www.burgerking.in/static/media/login-inactive.de941b20.svg" alt="" /></li>
            <span>LOGIN</span>
            <li><img src="https://www.burgerking.in/static/media/cart.3ff203fa.svg" alt="" /></li>
            <span>CART</span>
            </ul>     
            <img src="https://www.burgerking.in/static/media/search.03064934.svg" alt="" />

        </nav>
      
        {/* ===================================================== */}
        <center>
        <div id="showContent"  >
         <h1 id="change-loc">CHANGE LOCATION</h1>
         
         <div id="loc-body">
          
              <input type="search" placeholder="&#128269; Search for Your location " ></input>
              {/* <div id="location1">
                <img id="loc" src="https://www.burgerking.in/static/media/detect-location.054d5431.svg" alt="" />
                <span id="detect-loc">Detect location</span>
                </div> */}
         <button id="conf-loc">CONFIRM LOCATION</button>

         </div>
        </div>
        </center>
{/* ========================================================================== */}
                              

        </>
     )
}
export default NavBar;